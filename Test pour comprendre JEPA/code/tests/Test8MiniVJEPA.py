...
# Simulation d'une entrée vidéo avec des tokens spatio-temporels
dim = 256
batch_size = 2
num_context_video = 150  # Contexte vidéo visible (ex: frames passées)
num_target_video = 50    # Cubes vidéo masqués à prédire
...
# Les dimensions intègrent la dimension temporelle aplatie
context_inputs = torch.rand(batch_size, num_context_video, dim)
target_inputs_full = torch.rand(batch_size, num_context_video + num_target_video, dim)
mask_tokens_with_pos = torch.rand(batch_size, num_target_video, dim)# Instanciation des modèles
